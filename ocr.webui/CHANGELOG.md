...
- Создайте проект React с помощью create-vite: `npm init --yes vite@latest ocr.webui -- --template=react-ts  --no-rolldown --no-immediate`.
- Обновление `vite.config.ts` с портом.
- Создание файла проекта (`ocr.webui.esproj`).
